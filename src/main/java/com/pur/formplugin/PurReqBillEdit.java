...
public class PurReqBillEdit extends AbstractBillPlugIn {
    ...
    @Override
    public void propertyChanged(PropertyChangedArgs e) {//触发时机：表单模型某个字段值改变的时候就会触发此事件。
        String propertyName = e.getProperty().getName();
        ChangeData cd = e.getChangeSet()[0];

        ChangeData[] changeData = e.getChangeSet();
        int rowIndex = changeData[0].getRowIndex();//获取分录行数
        //this.getView().setEnable(false, "tpv_applydate");
//        if (propertyName.equals("tpv_applyqty")) {
//            //监听申请数量与建议采购单价
//            applyqtyAlter(cd);//申请数量变化，金额改变
//        } if (propertyName.equals("tpv_price")) {
//            priceChange(cd);//建议采购单价变化，金额改变
//        }  if (propertyName.equals("tpv_applyqty") || propertyName.equals("tpv_unit") || propertyName.equals("tpv_unitfield")) {
//            getBaseunitqty(rowIndex);//申请数量改变，计算基本单位数量
//        }  if (propertyName.equals("tpv_supplier")) {
//            this.getModel().deleteEntryData("tpv_entryentity");// 供应商改变清空分录
//        }  if (propertyName.equals("tpv_materia")) {
//            materielChange(cd);//物料变化
//        }  if (propertyName.equals("tpv_amount")) {
//            amountCollect();//金额变化汇总金额变化
//        }  if (propertyName.equals("tpv_currency") || propertyName.equals("tpv_tocurr") || propertyName.equals("tpv_exratetable") || propertyName.equals("tpv_exratedate")) {
//            amountCollect();//金额变化汇总金额变化
//        }  if(propertyName.equals("tpv_applier")){
//            carryDepartment();
//        }
        switch (propertyName) {
            //监听申请数量与建议采购单价
            ...
            case "tpv_unit":
            case "tpv_unitfield":
                getBaseunitqty(rowIndex);//基本计量单位改变，计算基本单位数量
                break;
            //监听供应商标识
            case "tpv_supplier":
                this.getModel().deleteEntryData("tpv_gjtt_purchase_cg");// 供应商改变清空分录
                break;
            //监听物料标识
            case "tpv_materia":
                materielChange(cd);//物料变化
    ...
    private void getBaseunitqty(Integer row) {
        ...
        if (num.compareTo(BigDecimal.ZERO) <= 0) {
            this.getModel().setValue("tpv_baseunitqtyfield", null, row);
        ...
        if (materiel != null && srcUnit != null && desUnit != null) {
            ...
            this.getModel().setValue("tpv_baseunitqtyfield", basenum, row);
        } else {
            this.getModel().setValue("tpv_baseunitqtyfield", BigDecimal.ZERO, row);
        }
    }
    ...
    private void amountCollect() {
        //获得分录
        DynamicObjectCollection entrys = this.getModel().getEntryEntity("tpv_gjtt_purchase_cg");
    ...
    public void afterDeleteRow(AfterDeleteRowEventArgs e) {
        ...
        if ("tpv_gjtt_purchase_cg".equals(entryName)) {
            amountCollect();
        }
    }
